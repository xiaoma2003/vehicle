...
class DynamicMap {
    ...
    drawNodes() {
        ...
        this.nodes.forEach(node => {
            ...
            if (node.name) {
                ...
                ctx.font = '11px Arial';
                ctx.fillText(node.name, pos.x, pos.y + radius + 28);
            }
        });
    }
    ...
    drawStaticVehicles() {
        ...
        this.vehicles.forEach((vehicle, index) => {
            ...
            ctx.fillText(vehicle.id, vx, vy - 20);
        });
    }
    ...
    drawAssignments() {
        ...
        this.assignments.forEach(assignment => {
            ...
            ctx.beginPath();
            ctx.arc(vehicleX, vehicleY, 14, 0, Math.PI * 2);
            ctx.fillStyle = vehicleColor;
            ctx.fill();
            ctx.strokeStyle = '#fff';
            ctx.lineWidth = 3;
            ...
            ctx.font = 'bold 11px Arial';
            ctx.textAlign = 'center';
            ctx.fillText(assignment.locomotive_id || '', vehicleX, vehicleY - 24);

            ctx.fillStyle = '#1a202c';
            ctx.font = '10px Arial';
            ctx.fillText(assignment.task_id || '', vehicleX, vehicleY - 38);
        });
    }
